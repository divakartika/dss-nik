...
employ_merge = pd.read_pickle('data/employ_merge.pkl')
...
input_select = col3.selectbox(
    ...
    options= employ_merge['department_name'].sort_values().unique())
...
min_age = input_slider[0]
max_age = input_slider[1]
...
# data: multivariate
employ_age = employ_merge[employ_merge['age'].between(left=min_age, right=max_age)]
...
col6.write(f'### Gender per Department, Age {min_age} to {max_age}')
